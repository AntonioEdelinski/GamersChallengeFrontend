    ...
    <v-row justify="center" align="center" style="min-height: 100vh;">
      <v-col cols="12" sm="8" md="6" lg="4">
        <v-card class="pa-6 quiz-card" outlined>
          <v-card-title class="text-h4 text-center">Quiz - Game Awards</v-card-title>
  ...
  methods: {
    ...
    async startQuiz() {
      try {
        ...
        const response = await fetch(`http://localhost:3000/quiz/questions?difficulty=${this.selectedDifficulty}`);
        if (!response.ok) {
...
.v-card-title {
  color: #000; /* Crna boja naslova kartice */
}
</style>
